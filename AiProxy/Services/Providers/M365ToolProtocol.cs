...
public static class M365ToolProtocol
{
        ...
        IReadOnlyList<OpenAiToolCall> previousCalls, IReadOnlyList<OpenAiToolResult> results)
    {
        ...
        {
            type = OpenAiConstants.ToolCalls.FunctionType,
            name = tool.Name,
        ...
        instruction.AppendLine("```");
        instruction.AppendLine("`name` must exactly match a declared function and `arguments` must be a JSON object. Do not call undeclared functions. After a tool result, either issue one next call in the same form or provide the final answer.");
    ...
    {
        call = null;
        if (!TryGetSingleFence(response, out var label, out var json))
            return false;
        ...
        try
        {
            ...
            if (root.ValueKind != JsonValueKind.Object || !TryGetName(root, out var name) ||
                !TryGetArguments(root, out var arguments))
                return false;
            ...
            if (callId.StartsWith(OpenAiConstants.ToolCalls.ProxyTodoCallIdPrefix, StringComparison.Ordinal) ||
                !declaredTools.Any(tool => string.Equals(tool.Name, name, StringComparison.Ordinal)))
                return false;

            call = new OpenAiProviderToolCall(callId, name, arguments.GetRawText());
            return true;
        }
        catch (JsonException)
        {
    ...
    private static bool TryGetArguments(JsonElement root, out JsonElement arguments)
    {
        arguments = default;
        ...
        try
        {
            using var argumentsDocument = JsonDocument.Parse(value.GetString() ?? string.Empty);
